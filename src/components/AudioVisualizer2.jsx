...
import { sumThisArray, reduceSizeArray, createNewArrayFromTo } from '../utils/arrayUtils';
...
function AudioVisualizer2({ dataFrequencyLeft, dataFrequencyRight }){
    ...
    const barRefsLeftTopSmall = useRef([])
    const lineRefsLeftTopSmall = useRef([])
    const dotRefsLeftTopSmall = useRef([])

    const barRefsLeftBottomSmall = useRef([])
    const lineRefsLeftBottomSmall = useRef([])
    const dotRefsLeftBottomSmall = useRef([])

    const [grid, setGrid] = useState(false)

    useEffect(() => {
        //const leftArray = createNewArrayFromTo(dataFrequencyLeft, 1000, 4096)
        // const leftArraySmall = createNewArrayFromTo(dataFrequencyLeft, 0, 4096)
        //const dataLeftTop = dataFormat(reduceSizeArray(dataFrequencyLeft, barsCount *2), barsCount, true);

        // const dataLeftTop = dataFormat(reduceSizeArray(leftArray, barsCount *2), barsCount, true);
        // console.log(dataLeftTop)

        // const dataLeftTopSmall = dataFormat(reduceSizeArray(leftArraySmall, barsCount *2), barsCount, true);
        // console.log(dataLeftTopSmall)
        
        const dataLeftTop = dataFormat(
            createNewArrayFromTo(reduceSizeArray(dataFrequencyLeft, 84), 20, 84), 42, true
        );
        const dataLeftTopSmall = dataFormat(
            createNewArrayFromTo(reduceSizeArray(dataFrequencyLeft, 84), 2, 44), 42, true
        );

        const dataLeftBottom = dataFormat(
            createNewArrayFromTo(reduceSizeArray(dataFrequencyLeft, 84), 20, 84), 42, false
        );
        const dataLeftBottomSmall = dataFormat(
            createNewArrayFromTo(reduceSizeArray(dataFrequencyLeft, 84), 2, 44), 42, false
        );
        
        const dataRightTop = dataFormat(
            createNewArrayFromTo(reduceSizeArray(dataFrequencyRight, 84), 20, 84), 42, true
        );
        const dataRightBottom = dataFormat(
            createNewArrayFromTo(reduceSizeArray(dataFrequencyRight, 83), 20, 84), 42, false
        );
        ...
        dataLeftTopSmall.forEach((bar, index) => {
            if (lineRefsLeftTopSmall.current[index] && dotRefsLeftTopSmall.current[index]) {
                lineRefsLeftTopSmall.current[index].style.transform = `scaleX(`+ barWidth(bar) + `) scaleY(1)`
                lineRefsLeftTopSmall.current[index].style.opacity = progressiveOpacity(bar)
                dotRefsLeftTopSmall.current[index].style.opacity = progressiveOpacity(bar) 
            }
        })
        ...
        dataLeftBottomSmall.forEach((bar, index) => {
            if (lineRefsLeftBottomSmall.current[index] && dotRefsLeftBottomSmall.current[index]) {
                lineRefsLeftBottomSmall.current[index].style.transform = `scaleX(`+ barWidth(bar) + `) scaleY(1)`
                lineRefsLeftBottomSmall.current[index].style.opacity = progressiveOpacity(bar)
                dotRefsLeftBottomSmall.current[index].style.opacity = progressiveOpacity(bar) 
            }
        })
        ...
        let intensityOfSound = intensityRatio(dataFrequencyLeft, dataFrequencyRight, dataFrequencyLeft.length * 256 * 2)
    ...
    return( 
        <Visualizer>
            <VisualizerPartLarge className='top left' barscolor={barsColor.current}>
                {initBars(barsCount, barRefsLeftTop, lineRefsLeftTop, dotRefsLeftTop)}
            </VisualizerPartLarge>
            <VisualizerPartLarge className='bottom left' barscolor={barsColor.current}>
                {initBars(barsCount, barRefsLeftBottom, lineRefsLeftBottom, dotRefsLeftBottom)}
            </VisualizerPartLarge>
            <VisualizerPartLarge className='top right' barscolor={barsColor.current}>
                {initBars(barsCount, barRefsRightTop, lineRefsRightTop, dotRefsRightTop)}
            </VisualizerPartLarge>
            <VisualizerPartLarge className='bottom right' barscolor={barsColor.current}>
                {initBars(barsCount, barRefsRightBottom, lineRefsRightBottom, dotRefsRightBottom)}
            </VisualizerPartLarge>

            <VisualizerPartSmall  className='top left' barscolor={barsColor.current}>
                {initBars(barsCount, barRefsLeftTopSmall, lineRefsLeftTopSmall, dotRefsLeftTopSmall)}
            </VisualizerPartSmall>
            <VisualizerPartSmall  className='bottom left' barscolor={barsColor.current}>
                {initBars(barsCount, barRefsLeftBottomSmall, lineRefsLeftBottomSmall, dotRefsLeftBottomSmall)}
            </VisualizerPartSmall>
            <VisualizerPartSmall  className='top right'>

            </VisualizerPartSmall>
            <VisualizerPartSmall  className='bottom right'>
            ...
            {grid &&
                <VisualizerGrid>
                    ...
                    <div className='grid-part2 top left'></div>
                    <div className='grid-part2 top right'></div>
                    <div className='grid-part2 bottom left'></div>
                    <div className='grid-part2 bottom right'></div>
                </VisualizerGrid>
            }
        </Visualizer>
    )
...
const Visualizer = styled.div`
    ...
    z-index:0;
`
const VisualizerPartLarge = styled.div`
    position:absolute;
    width:45%;
    height:50%;
    left:0%;
...
const VisualizerPartSmall = styled.div`
    width:25%;
    height:50%;
    position:absolute;
    display:flex;
